...
class mudclient:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.clients = []
        self.file_path = "data/mudclient/settings.json"
        self.json_data = dataIO.load_json(self.file_path)
        self.prefix = self.json_data["interactChar"]
    ...
    async def startConnection(self, ctx):
        ...
        if(hasSession != True):
            clientThread = client(self.bot, user, channel.id,self.json_data["Server"])
...
def check_files(): # This is how you check if your file exists and let's you create it
    system = {"Server" : {"Name" : "telehack", "IP" : "telehack.com"},
                "interactChar" : "#"}
    f = "data/mudclient/settings.json" # f is the path to the file
...
class client():
    ...
    async def start():
        while True:
            ...
            embed.set_author(name=self.user., url='http://stuff.com', icon_url='http://stuff.com')
